...
-- PASO 0: Quitar el candado de la cï¿½dula para que entre cualquier nï¿½mero
...
FROM (
    ...
    WHERE NUMERO IS NOT NULL
      AND NUMERO <= 999999999999999 -- Filtro anti-error ORA-01438
...
-- PASO 3: Importar a CICLISTAS (con filtro avanzado anti-basura en fechas)
...
FROM (
    ...
    WHERE NUMERO IS NOT NULL
      ...
      -- Asegura que solo procese formatos estrictos como 12/05/1990 o 12-05-90
...
CREATE OR REPLACE PACKAGE BODY PC_REGISTRO AS
    ...
    -- --------------------------------------------------------
    -- AD_REGISTRO: Inserta un nuevo registro de resultado.
    -- El trigger TR_REGISTROS_BI se encarga de:
    --   - Auto-numerar (numero)
    --   - Asignar fecha = SYSDATE
    --   - Asignar revision = 'Pendiente' si es NULL
    -- Las FK compuestas validan automaticamente:
    --   - Que el ciclista participe en la version
    --   - Que el segmento pertenezca a la version
    -- --------------------------------------------------------
    PROCEDURE AD_REGISTRO(
        xTIEMPO           IN NUMBER,
        xPOSICION         IN NUMBER,
    ...
    -- --------------------------------------------------------
    -- MOD_REGISTRO: Modifica solo comentario y revision.
    -- El trigger TR_REGISTROS_BU bloquea cualquier otro cambio.
    -- --------------------------------------------------------
    PROCEDURE MOD_REGISTRO(
        xID_CICLISTA      IN NUMBER,
        xNOMBRE_SEGMENTO  IN VARCHAR2,
    ...
    -- --------------------------------------------------------
    -- ELI_REGISTRO: Elimina un registro.
    -- El trigger TR_REGISTROS_BD bloquea si han pasado mas de 24h.
    -- Primero elimina las fotos hijas para no violar FK.
    -- --------------------------------------------------------
    PROCEDURE ELI_REGISTRO(
        xID_CICLISTA      IN NUMBER,
        xNOMBRE_SEGMENTO  IN VARCHAR2,
    ...
    -- --------------------------------------------------------
    -- AD_FOTO: Agrega una foto a un registro existente.
    -- El CHECK CK_FOTOS_URL valida el formato www.*.gif|pdf.
    -- --------------------------------------------------------
    PROCEDURE AD_FOTO(
        xURL              IN VARCHAR2,
        xID_REGISTRO      IN NUMBER
    ...
    -- --------------------------------------------------------
    -- CO_REGISTRO: Consulta un registro especifico usando la
    -- vista VI_REGISTRO_DETALLE para obtener datos enriquecidos.
    -- --------------------------------------------------------
    FUNCTION CO_REGISTRO(
        xID_CICLISTA      IN NUMBER,
        xNOMBRE_SEGMENTO  IN VARCHAR2,
    ...
    -- --------------------------------------------------------
    -- CO_TOP5_TIEMPOS: Retorna los 5 registros con tiempos 
    -- Esta es la consulta operativa del caso de uso.
    -- --------------------------------------------------------
...
-- CRUDOK (Pruebas exitosas que demuestran el buen diseno)
-- ============================================================
...
-- Cambie el ciclista 11 por el 16 (asegurate de que el 16 este inscrito en VE24)
BEGIN 
...
-- OK 3: Agregar una foto (Asumimos que el trigger le puso el numero de registro correspondiente, 
-- pero para ir a lo seguro y no depender del ID, podemos usar el ID del ciclista si lo tuvieras, 
-- aqui pondremos un ID de foto alto para evitar chocar).
-- OK 3: Agregar una foto (Usamos un ID alto como 100 para evitar chocar)
DECLARE
    v_num NUMBER;
BEGIN
...
-- NoOK 3: Intentar modificar el TIEMPO de un registro a traves del paquete.
--          Esto NO es posible porque MOD_REGISTRO solo acepta comentario y revision.
--          El diseno del paquete garantiza que campos criticos sean inmutables.
...
-- GRANT RL_PERSONA TO ; -- Otro parcipantes 
...
-- ============================================================
/* ==================================================================
   1. BLOQUE MAESTRO DE LIMPIEZA DE SANDBOX (98 y 99)
   Se borra en orden EXACTO de hijos a padres para no romper FKs
   ================================================================== */
...
-- OK 1: Registro de un resultado nuevo valido usando el paquete del Juez (PA_PERSONA)
BEGIN 
...
-- Buscamos dinamicamente el numero del registro que se genero en el OK 1
DECLARE
    v_num_reg NUMBER;
BEGIN 
...
-- Asumiendo que el registro ID 9999 no existe.
BEGIN 
...
-- Asumiendo que el campo revisión solo permite ciertas palabras como 'Oficial'.
BEGIN 
...
-- ============================================================
/* ============================================================
   PREPARACION DEL ENTORNO (Para que la Historia nunca falle)
   Limpia la basura y prepara a los corredores 11, 91 y 92
   ============================================================ */
-- 1. Limpiamos a Juan (12), Sofia (11) y los fantasmas (91, 92)
...
-- 2. Limpiamos la existencia de los fantasmas (Por si ejecutas esto 2 veces)
...
-- 3. Creamos a los ciclistas fantasmas para rellenar el Top 5
...
-- 4. Los inscribimos en la Vuelta Ecuador 2024
...
-- 5. Insertamos los tiempos
-- El 11 es para que el Juez lo borre en el Paso 8
