...
INSERT INTO SEGMENTOS VALUES ('SEG01', 'M', 'SALIDA1', 'ALTO1'); -- M: Monta�a
...
-- Nota: detalle_experiencia requiere un XML v�lido en Oracle
...
-- CASOS QUE FALLAN POR REGLAS DE DOMINIO (Aun no hay CHECKs, pero fallar�n por tipo de dato o NOT NULL)
...
-- Caso 2: IDN (Documento) menor al m�nimo de TNid (Aun se permite hasta que pongamos el CHECK)
-- INSERT INTO PARTICIPANTES VALUES (10, 'CC', 999, 'COLOMBIA', 'error@mail.com');
-- Se permite por ahora, pero fallar� cuando agreguemos la secci�n de ATRIBUTOS.

-- Caso 3: Categor�a fuera de rango (Aun se permite hasta que pongamos el CHECK)
...
-- Caso 1: Violaci�n de TIdn (debe ser >= 1111111111)
-- INSERT INTO PARTICIPANTES VALUES (6, 'CC', 999999, 'COLOMBIA', 'error@mail.com');

-- Caso 2: Violaci�n de TPais (debe ser MAY�SCULAS y solo letras/espacios)
-- INSERT INTO PARTICIPANTES VALUES (7, 'CC', 1555555555, 'colombia', 'error@mail.com');

-- Caso 3: Violaci�n de Tcorreo (formato de correo inv�lido)
-- INSERT INTO PARTICIPANTES VALUES (8, 'CC', 1666666666, 'COLOMBIA', 'correo_sin_punto@dominio');

-- Caso 4: Violaci�n de TCategoria en Ciclista (debe ser entre 1 y 5)
-- INSERT INTO CICLISTAS VALUES (1, DATE '1990-01-01', 9);

-- Caso 5: Violaci�n de TPeriodicidad en Carrera (valor no permitido)
-- INSERT INTO CARRERAS VALUES ('VCOLX', 'Vuelta X', 'COLOMBIA', 1, 'Z');

-- Caso 6: Violaci�n de TFoto (URL no empieza por www. o no termina en .gif/.pdf)
-- INSERT INTO FOTOS VALUES (10, 'http://fotos.com/bici.jpg', 1);

-- Caso 7: Violaci�n de TPuntuaci�n en Evaluaci�n (fuera de rango 1-5)
...
-- CICLO 1: Consultar los cinco segmentos con tiempos m�s cortos
SELECT 
...
-- ============================================================
-- CICLO 1:  Construcci�n:  nuevamente poblando
-- ============================================================
...
-- ============================================================
-- CICLO 2 CRUD - Caso de uso 2: Registrar Evaluaci�n
-- XDisparadores
-- ============================================================
/*
...
BEGIN
    ...
    IF :NEW.fecha < v_fecha_inicio OR :NEW.fecha > v_fecha_fin THEN
        RAISE_APPLICATION_ERROR(-20001, 'Error: La encuesta no est� activa en la fecha actual.');
    END IF;
EXCEPTION
...
BEGIN
    IF :OLD.id != :NEW.id OR
       ...
       :OLD.id_participante != :NEW.id_participante THEN
        ...
        RAISE_APPLICATION_ERROR(-20004, 'Error: Solo es posible modificar el estado, retroalimentaci�n o detalle de experiencia. Los campos cr�ticos son inmutables.');
    END IF;
END;
/
...
FOR EACH ROW
BEGIN
    RAISE_APPLICATION_ERROR(-20005, 'Error de Seguridad: No es posible eliminar una evaluaci�n una vez registrada en el sistema.');
END;
/
...
-- NoOK 1: Intentar modificar un campo inmutable (Puntuaci�n) - TR_EVALUACION_BU lo bloquea
-- UPDATE EVALUACIONES SET puntuacion = 1 WHERE id = 100;

-- NoOK 2: Intentar eliminar una evaluaci�n - TR_EVALUACION_BD lo bloquea
